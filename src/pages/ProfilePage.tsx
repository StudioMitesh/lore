  ...
  const updateProfileStats = async (tripsData: Trip[], entriesData: Entry[]) => {
    ...
    const continents = new Set(
      Array.from(allCountries).map((country) => {
        ...
        if (
          ['brazil', 'argentina', 'chile', 'peru', 'colombia'].some((c) => countryLower.includes(c))
        )
          return 'South America';
        if (
          ['uk', 'france', 'germany', 'spain', 'italy', 'portugal', 'netherlands'].some((c) =>
            countryLower.includes(c)
          )
        )
          return 'Europe';
        if (
          ['china', 'japan', 'korea', 'thailand', 'india', 'vietnam', 'indonesia'].some((c) =>
            countryLower.includes(c)
          )
        )
          return 'Asia';
                              ...
                              <img
                                src={
                                  selectedAvatar
                                    ...
                                    : profile.avatarUrl || '/placeholder.svg'
                                }
                                alt="Avatar preview"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {['twitter', 'instagram', 'linkedin', 'github'].map((platform) => (
                            <Input
                              ...
                              onChange={(e) =>
                                handleNestedChange('socialLinks', platform, e.target.value)
                              }
                              className="bg-parchment border-gold/30"
                            />
                          ))}
