...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        const loadProfileData = async () => {
            try {
                ...
                const tripsQuery = query(collection(db, 'trips'), where('uid', '==', user.uid));
    ...
    const updateProfileStats = async (tripsData: Trip[], entriesData: Entry[]) => {
        ...
        const continents = new Set(
            Array.from(allCountries).map((country) => {
                const countryLower = country.toLowerCase();
                if (
                    ['usa', 'united states', 'canada', 'mexico'].some((c) =>
                        countryLower.includes(c)
                    )
                )
                    return 'North America';
                if (
                    ['brazil', 'argentina', 'chile', 'peru', 'colombia'].some((c) =>
                        countryLower.includes(c)
                    )
                )
                    return 'South America';
                if (
                    ['uk', 'france', 'germany', 'spain', 'italy', 'portugal', 'netherlands'].some(
                        (c) => countryLower.includes(c)
                    )
                )
                    return 'Europe';
                if (
                    ['china', 'japan', 'korea', 'thailand', 'india', 'vietnam', 'indonesia'].some(
                        (c) => countryLower.includes(c)
                    )
                )
                    return 'Asia';
                if (
                    ['egypt', 'kenya', 'south africa', 'morocco', 'ethiopia'].some((c) =>
                        countryLower.includes(c)
                    )
                )
                    return 'Africa';
                if (['australia', 'new zealand', 'fiji'].some((c) => countryLower.includes(c)))
                    return 'Oceania';
                return 'Other';
            })
        );

        const totalPhotos = entriesData.reduce(
            (sum, entry) => sum + (entry.mediaUrls?.length || 0),
            0
        );
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-parchment-light rounded-2xl border border-gold/20 p-5">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <StatCard
                                        label="Total Trips"
                                        value={profile.stats.totalTrips || 0}
                                    />
                                    <StatCard label="Countries" value={profile.stats.countries} />
                                    <StatCard
                                        label="Active Days"
                                        value={profile.stats.activeDays || 0}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-3">
                                <Dialog open={editMode} onOpenChange={setEditMode}>
                                    ...
                                    <DialogContent className="max-w-3xl bg-parchment border border-gold/20 max-h-[90vh] overflow-y-auto">
                                        <DialogHeader>
                                            <DialogTitle className="font-display text-deepbrown">
                                                Edit Your Profile
                                            </DialogTitle>
                                        ...
                                        <div className="space-y-6 py-4">
                                            <div className="space-y-4">
                                                <h4 className="text-sm font-medium text-deepbrown">
                                                    Profile Images
                                                </h4>
                                ...
                                <div className="bg-parchment-light rounded-2xl border border-gold/20 p-5">
                                    ...
                                    <div className="space-y-4">
                                        {profile.interests && profile.interests.length > 0 && (
                                            <div>
                                                <h4 className="text-sm font-medium text-deepbrown mb-2">
                                                    Interests
                                                </h4>
                                                        ...
                                                        {profile.favoritePlaces
                                                            ...
                                                            .map((place, index) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <span className="text-xs text-deepbrown">
                                                                        {place}
                                                                    </span>
                                                                </div>
                                                            ))}
                        ...
                        <div className="space-y-8">
                            <Tabs defaultValue="overview" className="w-full">
                                    ...
                                    <div className="space-y-6">
                                        {trips.length > 0 && (
                                            <div className="bg-parchment-light rounded-2xl border border-gold/20 p-6">
                                                <h3 className="font-display text-xl font-medium text-deepbrown mb-4">
                                                    Recent Adventures
                                                </h3>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {trips.slice(0, 4).map((trip, index) => (
                                                        ...
                                                        >
                                                            <div className="flex items-start justify-between mb-2">
                                                                <h4 className="font-medium text-deepbrown">
                                                                    {trip.name}
                                                                </h4>
                                                                <span
                                                    ...
                                                    {recentEntries.map((entry, index) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <h4 className="font-medium text-deepbrown truncate">
                                                                    {entry.title}
                                                                </h4>
                                ...
                                <TabsContent value="timeline" className="mt-6">
                                    <div className="bg-parchment-light rounded-2xl border border-gold/20 p-6">
                                        <h3 className="font-display text-xl font-medium text-deepbrown mb-6">
