...
export default function EntryDisplayPage() {
  ...
  const getEntryIcon = (type: string) => {
    switch (type) {
      ...
      case "artifact": return <Archive className="h-5 w-5" />
  ...
  const handlePreviousImage = () => {
    ...
    setSelectedImageIndex(selectedImageIndex > 0 ? selectedImageIndex - 1 : entry.mediaUrls.length - 1)
  }

  const handleNextImage = () => {
    if (selectedImageIndex === null || !entry?.mediaUrls) return
    setSelectedImageIndex(selectedImageIndex < entry.mediaUrls.length - 1 ? selectedImageIndex + 1 : 0)
  }
                ...
                <Card className="border-gold/20 bg-parchment-light">
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 text-sm text-deepbrown/70">
                      <p>Coordinates: {entry.coordinates.lat.toFixed(6)}, {entry.coordinates.lng.toFixed(6)}</p>
                    </div>
                  </CardContent>
                </Card>
              )}
              ...
              <Card className="border-gold/20 bg-parchment-light">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">{format(new Date(entry.timestamp), "MMM d, yyyy")}</span>
                  </div>
                  <div className="flex justify-between items-center">
