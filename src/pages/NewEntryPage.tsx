...
export default function NewEntryPage() {
  ...
  const getContinent = (country: string): string => {
    ...
    if (['brazil', 'argentina', 'chile', 'peru', 'colombia'].some((c) => countryLower.includes(c)))
      return 'South America';
    if (
      ['uk', 'united kingdom', 'france', 'germany', 'spain', 'italy', 'netherlands'].some((c) =>
    ...
    if (
      ...
      return 'Asia';
    if (
      ['egypt', 'kenya', 'south africa', 'morocco', 'nigeria'].some((c) => countryLower.includes(c))
    )
      return 'Africa';
        ...
        {selectedTrip ? (
          ...
          >
            <SelectTrigger className="bg-gold/20 border-gold/50 text-deepbrown">
              <SelectValue
                placeholder={isLoadingTrips ? 'Loading trips...' : 'Select a trip to associate'}
              />
            </SelectTrigger>
