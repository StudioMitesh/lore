...
export default function NewEntryPage() {
    ...
    const uploadImages = async (images: File[]): Promise<string[]> => {
        ...
        const uploadPromises = images.map(async (image, index) => {
            const imageRef = ref(
                storage,
                `entries/${user!.uid}/${Date.now()}_${index}_${image.name}`
            );
        ...
        try {
            ...
            if (profileSnap.exists()) {
                ...
                const countries = new Set(
                    [profile.stats.countries, newEntry.country].filter(Boolean)
                );
                const continents = new Set(
                    [profile.stats.continents, getContinent(newEntry.country)].filter(Boolean)
                );
    ...
    const getContinent = (country: string): string => {
        ...
        if (
            ['uk', 'united kingdom', 'france', 'germany', 'spain', 'italy', 'netherlands'].some(
                (c) => countryLower.includes(c)
            )
        )
            return 'Europe';
        if (
            ['china', 'japan', 'korea', 'thailand', 'india', 'vietnam', 'singapore'].some((c) =>
                countryLower.includes(c)
            )
        )
            return 'Asia';
    ...
    const loadUserTrips = async () => {
        ...
        try {
            ...
            const trips = tripsSnapshot.docs.map(
                (doc) =>
                    ({
                        id: doc.id,
                        ...doc.data(),
                ...
                <div className="container max-w-4xl mx-auto px-4 sm:px-6">
                    ...
                    <div className="space-y-8">
                            ...
                            <CardContent className="space-y-4 sm:space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <div
                                            className="relative search-input-container w-full"
                        ...
                        >
                            ...
                            {showTripAssociation
                                ? 'Hide Trip Association'
                                : 'Associate with a Trip (Optional)'}
                        </Button>
                        ...
                        <Card className="border-gold/20 bg-parchment-light">
                            <CardHeader>
                                <CardTitle className="text-xl text-deepbrown">
                                    Your Story *
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                ...
                                {imagePreviewUrls.length > 0 && (
                                    <div className="mt-3 sm:mt-4 space-y-2 sm:space-y-3">
                                        {images.map((file, index) => (
                                            <div
                                                key={index}
                                                className="flex items-center justify-between p-2 bg-parchment rounded border border-gold/20"
                                            >
                                                <span className="text-sm text-deepbrown truncate">
                                                    {file.name}
                                                </span>
                                                <Button
                                                    variant="ghost"
                        ...
                        <Card className="border-gold/20 bg-parchment-light">
                            <CardHeader className="pb-4 sm:pb-6">
                                ...
                                <p className="text-xs sm:text-sm text-deepbrown/70 mt-1 sm:mt-2">
                                    Search above or click on the map to set the exact location of
                                    your adventure. Selected locations will appear as markers.
                                </p>
                            </CardHeader>
                            <CardContent>
